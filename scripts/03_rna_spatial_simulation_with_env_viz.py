...
# Spatial RNA-world simulation with environment visualization
...
# Environment: values from -1 (hostile) to +1 (nutrient-rich)
ENV_GRID = np.random.uniform(-1, 1, (GRID_SIZE, GRID_SIZE))
...
# Set up interactive plot with two subplots
plt.ion()
...
for gen in range(GENERATIONS):
    ...
    # Update molecule counts and average environment influence
    counts = np.array([[len(grid[y][x]) for x in range(GRID_SIZE)] for y in range(GRID_SIZE)])
    avg_env = (ENV_GRID * counts).sum() / total if total > 0 else 0
