...
# Generate and smooth environment
...
for gen in range(1, GENERATIONS + 1):
    ...
    avg_energy = np.mean([e for _, e in molecules]) if total else 0
    unique_count = len(set(rna for rna, _ in molecules))
    avg_env = (ENV_GRID * np.array([[len(grid[y][x]) for x in range(GRID_SIZE)] for y in range(GRID_SIZE)])).sum() / total if total else 0

    # Update molecule map and stats
    counts = np.array([[len(grid[y][x]) for x in range(GRID_SIZE)] for y in range(GRID_SIZE)])
    mol_im.set_data(counts)
    mol_ax.set_title(f'Generation {gen}')
    stats = (f"Gen: {gen} | Total: {total} | Ribozymes: {ribo_count} | "
             f"AvgE: {avg_energy:.2f} | Unique: {unique_count} | "
             f"Off: {offspring_count} | EnvAvg: {avg_env:.2f}")
...
# Interactive hover for final state
cursor = mplcursors.cursor(mol_im, hover=True)
@cursor.connect("add")
def on_add(sel):
    row, col = sel.index
    cell = grid[row][col]
    total = len(cell)
    energies = [e for (_, e) in cell]
    sequences = [rna for (rna, _) in cell]
    avg_e = np.mean(energies) if total else 0
    counts = Counter(sequences)
    common = counts.most_common(3)
    common_str = ", ".join(f"{seq}×{cnt}" for seq, cnt in common)
    info = (
        f"Cell ({row},{col})\n"
        f"Count: {total}\n"
        f"Avg Energy: {avg_e:.2f}\n"
        f"Unique Seq: {len(counts)}\n"
        f"Top: {common_str}"
    )
